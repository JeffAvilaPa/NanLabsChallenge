...
| Including basic test coverage metrics using `cypress-coverage`                           | ✅     |
| Including Postman examples for REST API validation                                       | ✅     |
| Using GitHub Actions or any CI to run tests on pull requests                             | ✅     |
| Including tests that validate behavior after state changes (e.g., clicks, submissions)   | ✅     |
...
| API2 | POST to All Products List          | POST   | [productsList](https://automationexercise.com/api/productsList)   | 405           | Validates that POST is not allowed.                |
...
| API6 | POST Search Product (no parameter) | POST   | [searchProduct](https://automationexercise.com/api/searchProduct) | 400           | Validates error when the parameter is missing.     |
...
| API8 | POST Verify Login (missing email)  | POST   | [verifyLogin](https://automationexercise.com/api/verifyLogin)     | 400           | Validates error when email or password is missing. |

---

## Contact

If you have any questions or suggestions for improvements, feel free to reach out or open an issue in the repository.
